...
Here, *h* is a linear function: $$ \begin{displaymath} h_\theta (x) = \theta_0 + \theta_1 x \end{displaymath} $$ . `\theta_0`  and `\theta_1` are the parameters. To find the best model is to find the parameters that minimize the sum of the squared difference between $$ h_\theta (x^(i)) $$ and `y^(i)`. The cost functiom is then the squared error function.
